...
RECEIVERSTATE * initializeReceiver(EC_GROUP * group, EC_POINT * g1, EC_POINT * g2, BN_CTX * ctx){
    ...
    generateECEGKey(group, g1, s->sk, s->pk, ctx);

    //Initialize V to E(1)
    BIGNUM * one = BN_new();
    BN_one(one);
    encryptECEG(group, g1, g2, s->pk, one, s->V1, s->V0, ctx);
    BN_free(one);

    //Initialize D to E(0)
    BIGNUM * zero = BN_new();
    BN_zero(zero);
    encryptECEG(group, g1, g2, s->pk, zero, s->D1, s->D0, ctx);
    BN_free(zero);
...
int receiverStep1(EC_GROUP * group, EC_POINT * g1, unsigned int x, RECEIVERSTATE * s, BN_CTX * ctx){
    ...
    //c
    EC_POINT_mul(group, c0, NULL, g1, r0, ctx);
    if( x == 1 )
        EC_POINT_add(group, c0, c0, s->V0, ctx);

    EC_POINT_mul(group, c1, NULL, s->pk, r0, ctx);
    if( x == 1 )
        EC_POINT_add(group, c1, c1, s->V1, ctx);
    
    //c'
    EC_POINT_mul(group, cp0, NULL, g1, r1, ctx);
    if( x == 0 )
        EC_POINT_add(group, cp0, cp0, s->V0, ctx);

    EC_POINT_mul(group, cp1, NULL, s->pk, r1, ctx);
    if( x == 0 )
        EC_POINT_add(group, cp1, cp1, s->V1, ctx);

    //d
    EC_POINT_mul(group, d0, NULL, g1, r2, ctx);
    if( x == 1 )
        EC_POINT_add(group, d0, d0, s->D0, ctx);

    EC_POINT_mul(group, d1, NULL, s->pk, r2, ctx);
    if( x == 1 )
        EC_POINT_add(group, d1, d1, s->D1, ctx);
    ...
    EC_POINT_add(group, s->T1, c1, dp1, ctx);

    EC_POINT_add(group, s->U0, cp0, d0, ctx);
    EC_POINT_add(group, s->U1, cp1, d1, ctx);
...
int receiverStep3(EC_GROUP * group, EC_POINT * g1, unsigned int x, RECEIVERSTATE * s, EC_POINT * X0, EC_POINT * X1, EC_POINT * Y0, EC_POINT * Y1, BN_CTX * ctx){
...
unsigned char * receiverPRF(EC_GROUP * group, RECEIVERSTATE * s, BN_CTX * ctx){
...
unsigned char * senderPRF(EC_GROUP * group, EC_POINT * g2, SENDERSTATE * s, int * x, int length, BN_CTX * ctx){    //Calculate g2^\prod{b_i where x_i = 0} * \prod{a_i where x_i = 1}
    EC_POINT * iprf = EC_POINT_new(group);
