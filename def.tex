...
delegating PRF computation for strings $x=(x_1\ldots{}x_\ell)$ sharing
the same fixed prefix. That is, a party knowing key $K$ specifies a
prefix $x^*=(x^*_1\ldots{}x^*_i)$, computes $K'$ and $\iprf'$, and
gives $(\iprf',K')$ to another party. This party is then capable to
evaluate bit strings $x$ having the same prefix $x^*$.
