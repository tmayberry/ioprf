...
\usepackage{amsmath,parskip,enumitem,url}
...
\begin{document}
An iterated oblivious pseudo-random function $\ioprf$ is a protocol
$\proto$ between a sender $S$ with input $K$ and receiver $R$
with input $(b_1,\ldots,b_\ell)$ with the following properties.

\begin{itemize}
\item Protocol $\proto$ outputs a sequence of values
  $(v_1,\ldots,v_\ell),|v_i|=\lambda$ to $R$ and nothing to $S$.

\item Consider the $v_i$ as random variables describing $R$'s output
  for a fixed input $(b_1,\ldots,b_\ell)$ and random key $K$.
   
  We require $\forall{}(b_1,\ldots,b_\ell)\in\{0,1\}^\ell,\forall{}i\in\{1,\ldots,\ell\}:$
  $$|Pr[\A(v_1,\ldots,v_{i-1},v_i)]-Pr[\A(v_1,\ldots,v_{i-1},U_\lambda)]|=\epsilon(\lambda),$$
  where $U_\lambda$ is the random variable describing uniformly random
  bit strings of length $\lambda$.

\item For all $\lambda$ and $(b_1,\ldots,b_\ell)$, there exists a
  simulator $\myS(\ioprf_K(b_1,\ldots,b_\ell))$ who can simulate $R$'s
  view of the execution of $\pi$
\item Oblivious
