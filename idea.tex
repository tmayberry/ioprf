...
\fixme{Add description that PRF outputs must be hashed to become real
  output of the PRF. This is necessary for the leftover hash lemma and
  will also allow ROM proofs.}
...
$\iprf(b_1,\ldots,b_\ell)$: output a vector $\vec{v}$ of length $\ell$ where $v_i = ( \prod_{j=1}^{i} r_j^{b_j}s_j^{1-b_j}) \cdot{}G$
