...
\newcommand{\ot}[0]{\mathsf{i}\mathsf{OT}}
...
Initialization: choose a key $\vec{r}$ by sampling $\ell$ random scalars $r_i\getr\Z^*_q$

$\iprf(b_1,\ldots,b_\ell)$: output a vector $\vec{v}$ of length $\ell$ where $v_i = G \cdot \prod_{j=1}^{i} r_i^{b_i}$
...
\item For each $1 \leq i \leq \ell$, $R$ executes $\ot(b_i, a_i, a_i \cdot r_i)$ and stores the result as $z_i$
\item $S$ sends to $R$ the vector $\vec{C}$ where $\forall 1 \leq k \leq \ell$, $C_i =  G \cdot (1/\prod_{i=1}^{j} a_i)$
\item $R$ recovers each iterated PRP output by calculating $v_i = C_i \cdot \prod_{j=1}^{i} z_i$
\end{itemize}
...
&= G \cdot (1/\prod_{j=1}^{i} a_i) \cdot \prod_{j=1}^{i} z_i \\
&= G \cdot (1/\prod_{j=1}^{i} a_i) \cdot \prod_{j=1}^{i} r_i^{b_i} \cdot a_i \\
&= G \cdot \prod_{j=1}^{i} r_i^{b_i}
\end{aligned}
\end{equation}
