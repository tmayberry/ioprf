...
\label{our-ioprf}
\todo{ for one-sided security, we use the OT-based solution.}

\subsubsection{Preliminaries}
Let there be two generators $g_1,g_2$ of prime order $p$ DDH group
$\myG$. Nobody knows the discrete log of one generator $g_i$ to the
...
A Pedersen commitment $\com_{g}(m)\in\myG$ to message $m\in\Z_p$ is defined as
$\com_{g}(m)=g_1^r\cdot{}g^m$, where $r\getr\Z_p$, and $g\neq{}g_1$ is
a generator of $\myG$.  To open $\com_{g}(m)$, reveal $(m,r)$.


\subsubsection{$\proto$ Initialization}
The sender in $\proto$ knows secret key
$K=((\alpha_1,\beta_1),\ldots,(\alpha_\ell,\beta_\ell))$.  For some
input string $x=(x_1\ldots{}x_\ell)$, we define the output of $\proto$
for the receiver as $(v_1,\ldots,v_\ell)$ with
$v_i=g_2^{\prod_{j=1}^{i}(\alpha_j{}x_j+\beta_j(1-x_j))}$.
...
\item The receiver computes one $\iprf$ output value
...
\fixme{We cannot use Fiat-Shamir transform and replace $e$, as we use
...
\label{pomult}
Given two elements $(A,B)$ of DDH group $\myG$, such as an Elgamal
