...
these are encryptions of $1$ and $0$ (see \S\ref{enc_one},
\S\ref{enc_zero}). The sender computes Pedersen commitments
...
\subsubsection{Proof of Encryption of $1$}
\label{enc_one}
The proof that Elgamal ciphertext
$c=(c[0],c[1])=(g^r_1,pk^r\cdot{}g_2)$ is indeed an encryption of $1$
works as follows.
...
\subsubsection{Proof of Encryption of $0$}
\label{enc_zero}
\fixme{also Proof of DLOG Equivalence/DDH tuple/ --- otherwise change introductory text}

You can prove that tuple $(u_1=g_1,u_2=g_1^r,u_3=g_1^x,u_4=g_1^{xr})$ is a DDH
tuple, i.e., you show that $\log_{u_1}{u_2}=\log_{u_3}{u_4}$.
