...
Recall that a three-move ZK proof comprises messages $(a,e,z)$, where
first message $a$ is a commitment from $P$ sent to $V$, $e$ is $V$'s
challenge sent to $P$, and $z$ is the final message sent from $P$ to
$V$.
...
$\com_g(e)$.  After $P$ has sent their final message $z$, $P$ also
reveals $\rho$ to $V$. Only if both is correct, the last ZK proof
message $z$ matches $P$'s commitment $a$ and challenge $e$, and $\rho$
...
instead, we replace first message $a$ of the regular three-move
protocol by a Pedersen commitment $\com_{g_r}(a)$ from prover $P$ to
$a$. Here, the commitment uses generator $g_r=H(r)$ for a random $r$
...
protocol together with opening commitment $\com_{g_r}(a)$. Verifier $V$
accepts, if the commitment matches and $z$ of the regular protocol
matches $e$.

In the Random-Oracle model, this technique allows a simulator $\myS$
to learn $e$ before sending first message $a$ of the three-move
...
So, before $\myS$ sending any commitment, $V$ starts by sending
$r\getr\Z_p$, and $\myS$'s commitment will be computed with respect to
some $g_r$ where $\myS$ knows trapdoor $\rho_r$.

Thus, instead of commitment $\com_{g'}(a)$, $\myS$ commits at the
beginning to some arbitrary $r'$ by sending $\com_{g_r=H(r)}(r')$. As
soon as $\myS$ has received $e$, they can fake the last message of the
...
$\com_{H(r)}(a)$, and $V$ opens $\com_g(e)$.  Finally $P$ opens
$\com_{H(r)}(a)$ to reveal $a$, and sends last message $z$ of the
three-move protocol.
