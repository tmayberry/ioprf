...
the receiver as
$\iprf_K(x)=g_2^{\prod_{i=1}^{\ell}(a_ix_i+b_i(1-x_i))}$.
...
plaintext they want (equivocal).  We use this property for witness
...
\subsubsection{Proof of Knowledge of Pedersen Plaintext}
...
\subsubsection{Arithmetics with Pedersen Commitments}
...
\item $P$ proves that $\com_{g}(1)$ is a commitment to $1$ (see
  above).
  \end{enumerate}
\item \fixme{Missing: proof of either 0 or 1.}
\end{itemize}
